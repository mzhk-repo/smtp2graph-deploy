...
2026-07-29 — Task 3.2 policy/storage changes are now represented by control-plane asset `005-smtp-policy-and-storage-guards.patch`. All five v1.1.5 assets replayed from clean upstream in an isolated worktree; local build, 6 unit tests and 31 receive tests passed. The M365 suite is explicit opt-in through `--env-file`, so local patch qualification does not implicitly read build-plane `.env` or perform external calls. Successful automation removes its temporary worktree and `upgrade/vX.Y.Z` branch; failed runs preserve them for review. The current local build-plane diff is still disposable and must not be promoted directly. The control-plane runtime template now also enforces SMTP AUTH plus non-empty source-IP/sender allowlists and strict Docker Secret TSV user records with lowercased sender checks and redacted diagnostics. Failed payload retention now defaults to dry-run, purges only files at least seven days old from the validated fixed `failed` child and never touches queue data.
