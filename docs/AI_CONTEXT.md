...
- Certificate bootstrap now requires env-controlled minimum Certbot and DNS-Cloudflare plugin versions during explicit deploy apply. Missing PEM values create only temporary mode-`0600` ignored staging for manual SOPS handoff; Graph public-certificate Entra onboarding and every certificate rotation remain operator-runbook steps.
...
- Task 5.1b dev/prod Swarm IaC is defined. The canonical stack derives required `smtp2graph_dev=true` or `smtp2graph_prod=true` placement from `DEPLOY_ENVIRONMENT`; host-side `SERVER_ENV=dev|prod` from `/etc/environment` must match. The idempotent legacy migration from `smtp2graph_nonproduction` and `/srv/smtp2graph/non-production` is explicit `--apply` only and refuses a running gateway or unsafe storage paths. Production host mutation additionally requires an approval context; no production host state was changed.
- Task 5.2 dev/prod Swarm orchestration is complete in the control plane. `scripts/deploy-orchestrator-swarm.sh` strictly parses only allowlisted stack inputs, env-derived immutable digest and Secret names without `source`; development may use local ignored `.env` fallback, while `.env.dev.enc` remains a SOPS reconciliation source. Every normal `--deploy --apply` content-addressably reconciles the selected SOPS secret values, atomically reloads the names-only mapping and then submits the stack: a changed Secret gets a new immutable name, changes the Swarm task template and produces a replacement task without `docker service update --force`; unchanged content remains a no-op task deploy. Before every deploy or rollback it invokes the shared idempotent `init-storage.sh`, which validates or creates the reviewed storage root and its direct queue/failed children. The initializer is included in the immutable Config revision hash, so storage-contract changes produce a normal declarative Swarm task update without `docker service update --force`. Production requires an explicit env-file, label, tag, approval context and matching exact control-plane SHA. Fake-Docker regression covers secret rotation, dev idempotency, digest-only deploy and production guards; no production deployment was performed.
- Task 5.3 separates build from deploy. Build-plane PR/dev/tag paths are thin shared-CI callers without local Action/image `uses:`; shared CI/CD owns the pinned toolchain while build source supplies only npm validation. Tags retain exact-digest Trivy, CycloneDX and checksum evidence without overwriting a Release. A protected control-plane manual dispatch accepts only an immutable digest and invokes the reviewed Swarm adapter; production additionally requires an approver's manual Release-evidence attestation. `mzhk-repo/shared-workflows/.github/workflows/shared-ci-cd.yml@main` remains the sole owner-approved mutable-reference exception.
- Task 5.4 single-release development smoke rehearsal is complete. The exact fork digest `ghcr.io/mzhk-repo/smtp2graph-build@sha256:3d5069229059411988884913b6afba2d198c43e2440c1037e44d6aa54f172f65` has independently reviewed immutable build-plane release artifacts. Fresh deploy, no-op declarative redeploy with the same desired task ID, and repeated read-only smoke passed: exactly one desired task is `Running`, SMTP returns `220`, reviewed Secret mount modes and live encrypted-overlay/nftables policy checks pass. The SOPS-backed deploy loader decrypts only into `/dev/shm`, then requires and overlays the complete private names-only Secret mapping, so rotation never relies on manually copied `*_SECRET_NAME` values. `GRAPH_SENDER_MAILBOX` is the single runtime sender and derives the global SMTP allowlist; `SMTP_USERS_TSV` retains its independent per-user sender scope. `SMTP2GRAPH_IMAGE_DIGEST` is the only deployment image input; release evidence remains in immutable build-plane artifacts. Upgrade, queue migration and rollback rehearsal remain deferred until a second independently reviewed fork release and an explicit operator compatibility assessment. ADR-0008 retains the At-Least-Once model for future rollback/recovery.
- Task 6.1 accepted development integration scope is complete. Gateway format delivery passed for plain text, HTML/Unicode, To/CC, Reply-To, regular/inline attachments and a BCC-envelope case with read-only recipient evidence on 2026-08-12. Grafana Alerting test notification delivery through STARTTLS and the encrypted `smtp2graph_internal_enc` overlay passed on 2026-08-13 after the DNS alias, overlay source-CIDR and host firewall precedence corrections. The real Moodle VM path/delivery and remaining Matomo/client-profile evidence remain explicitly deferred follow-up items; they are not claimed as passed.
- Task 6.2 disposable failure, durability, and retention suite is complete. The SMTP smoke test suite was enhanced to support STARTTLS and a dedicated failure durability test runner was introduced (`tests/acceptance/failure/run.sh`). Under simulated Graph conditions (Access Denied, 5xx server errors, 401 unauthorized, and Retry-After), the gateway correctly retries or relocates payloads to `failed/` while respecting the storage root and retention rules. The fork provides an HTTP `ObservabilityServer` exposing `/livez`, `/readyz`, and Prometheus `/metrics`, alongside standardized JSON-structured logging.
- Task 7.1 is accepted for the control-plane scope. Live development evidence confirms `/livez`, `/readyz`, bounded Prometheus metrics and structured JSON logs after declarative deploy. Port `9464` is internal to the encrypted overlay; the Swarm healthcheck uses `/readyz`, and the Docker `local` driver retains at most 30 files of 10 MiB rather than a time-based retention SLA. VictoriaMetrics overlay attachment and import of `deploy/monitoring/smtp2graph-scrape.yml` belong to Task 7.2. Entra credential-expiry metric and alert are explicitly out of scope; TLS certificate expiry remains a Task 7.2 alert signal.
- Task 7.2 is accepted for the deployed monitoring integration. VictoriaMetrics joins the encrypted gateway overlay and scrapes `smtp2graph_gateway:9464/metrics` with stable `env=prod`, `service=smtp2graph`, `component=gateway` labels; port `9464` remains unexposed on the host. The monitoring control plane owns the SMTP2Graph Gateway dashboard, metric alerts and a single synthetic runner. The runner uses STARTTLS through overlay DNS alias `gateway`, proves SMTP acceptance plus an increased Graph-success delivery counter, exports a configurable freshness threshold through Node Exporter, and sends alert notifications through an independent external SMTP path. Synthetic credentials remain a versioned monitoring-stack Docker Secret. Task 7.3–7.5 and Gate D remain pending.
- Task 7.3 control-plane implementation is complete but live recovery evidence remains pending. `backup.sh` creates checksum-verified allowlisted control-plane archives locally and through rclone using destinations from SOPS-encrypted `env.*.enc`, retaining 7 local and 30 cloud copies. `restore.sh` extracts only to a new/empty target and requires later secret reconciliation, bootstrap, deploy and synthetic verification. Archives exclude every queue, failed, log and Secret payload. ADR-0010 supersedes the mandatory RTO consequence of ADR-0007: elapsed recovery time is recorded but has no fixed acceptance threshold; configuration RPO remains 24 hours through Ansible-scheduled manual procedure.
- Moodle gateway-side preflight passed on 2026-08-12: trusted TLS hostname validation succeeded, SMTP AUTH was denied before STARTTLS and the configured `moodle` user authenticated after STARTTLS. The real Moodle VM network path, client configuration and controlled recipient delivery remain required before marking its Task 6.1 profile complete.
...
- Operational and security/audit logs are bounded by the Docker `local` driver to 30 files of 10 MiB; no time-based retention SLA applies. Failed payloads are retained for at most 7 days.
...
- VictoriaMetrics + Grafana is the selected monitoring stack. TLS-certificate expiry alerts must warn at 30 days and be critical at 7 days; Entra credential-expiry alerts are out of scope.
