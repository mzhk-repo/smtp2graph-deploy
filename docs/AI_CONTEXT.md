...
- Task 5.1b dev/prod Swarm IaC is defined. The canonical stack requires env-derived `smtp2graph_dev=true` or `smtp2graph_prod=true` according to its environment contract; host-side `SERVER_ENV=dev|prod` from `/etc/environment` must match. The idempotent legacy migration from `smtp2graph_nonproduction` and `/srv/smtp2graph/non-production` is explicit `--apply` only and refuses a running gateway or unsafe storage paths. Production host mutation additionally requires an approval context; no production host state was changed.
