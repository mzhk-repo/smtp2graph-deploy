...
- Task 5.4 наразі обмежено single-release development smoke rehearsal. Exact fork digest `ghcr.io/mzhk-repo/smtp2graph-build@sha256:3d5069229059411988884913b6afba2d198c43e2440c1037e44d6aa54f172f65` має independently reviewed release artifacts у `deploy/config/queue-compatibility.yml`; fresh deploy, no-op redeploy і read-only smoke можуть виконуватись лише на authorised development host. Upgrade, queue migration і rollback rehearsal deferred до другого independently reviewed queue-compatible fork release. ADR-0008 зберігає At-Least-Once model для майбутнього rollback/recovery.
...
2026-08-07 — Task 5.4 now permits only a single-release development deploy and read-only smoke rehearsal for fork digest `sha256:3d506…f172f65`; its release artifacts and checksums were independently verified. Upgrade, queue migration and rollback rehearsal are explicitly deferred until a second independently reviewed compatible fork release exists. No live stack, Docker Secret, queue or Microsoft 365 action was performed.
