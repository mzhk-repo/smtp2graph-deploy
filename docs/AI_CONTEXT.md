...
- Task 6.2 disposable failure, durability, and retention suite is complete. The SMTP smoke test suite was enhanced to support STARTTLS and a dedicated failure durability test runner was introduced (`tests/acceptance/failure/run.sh`). Under simulated Graph conditions (Access Denied, 5xx server errors, 401 unauthorized, and Retry-After), the gateway correctly retries or relocates payloads to `failed/` while respecting the storage root and retention rules. Real-time observability is established via an HTTP `ObservabilityServer` exposing `/livez`, `/readyz`, and Prometheus `/metrics`, alongside standardized JSON-structured logging.
