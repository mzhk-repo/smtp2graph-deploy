...
2026-07-28 — Task 2.5 functional evidence for fork revision `0fbb699` is complete: runtime, protocol/MIME, queue durability/restart, Graph retry/permanent-error handling, client-secret, certificate, positive delivery, denied-mailbox and display-name qualification passed. Formal functional Gate B owner approval remains pending; GHCR build/push, exact digest and supply-chain evidence remain deferred to Task 5.3. Exchange Online uses mailbox display name `noreply`, so production retains the approved per-service mailbox model. The inactive build-plane CI/CD caller and versioned patch automation remain without deployment authority.
