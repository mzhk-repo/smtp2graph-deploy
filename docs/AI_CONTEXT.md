...
- Task 4.3 SOPS + age control-plane implementation is complete for local/non-production validation: `.sops.yaml` protects `env.dev.enc` and `env.prod.enc`; the shared reconciler decrypts only in `/dev/shm`, creates deterministic immutable Graph/SMTP/TLS Docker Secret names and atomically updates a names-only mapping. `--apply` is fail-closed for production and has not created Docker Secrets. `env.dev.enc` retains a placeholder for `SMTP_USERS_TSV`, so real reconciliation remains blocked until that credential is approved; age recovery custody and CI trust also remain owner decisions.
...
2026-07-30 — Task 4.3 added SOPS + age encrypted environment files and a shared non-production Docker Secret reconciler. Decryption is confined to `/dev/shm`; versioned mappings contain names only; production apply is refused. The local encrypted source deliberately retains an SMTP users placeholder, so no real Docker Secret, deployment or production action has occurred.
