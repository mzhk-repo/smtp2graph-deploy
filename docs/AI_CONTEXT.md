...
- Task 3.1 local runtime wrapper is complete. `scripts/entrypoint.sh` renders the reviewed `deploy/config/gateway-config.yml.template` only into an existing tmpfs mount, reads mounted Docker Secret files, strictly parses an optional allowlisted non-secret input file without `source`, and fails closed for missing, writable or unsuitable secret files. Shell tests cover certificate and client-secret modes plus missing-secret, weak-permission and unsafe-input cases.
...
- Control plane містить versioned patch bundle і manual upgrade script. Він накладає reviewable assets на isolated local upstream worktree, запускає regressions і не виконує push, PR або deploy. Upstream drift/conflict fail-closed; M365 suite може завершитися documented `PARTIAL` за відсутності ignored test inputs.
...
2026-07-28 — Task 3.1 implements the local safe runtime wrapper and non-secret SMTP2Graph YAML template. The wrapper accepts only allowlisted non-secret runtime inputs without sourcing a file, requires mounted Docker Secret files that are regular and not writable by group/other, and renders `config.yml` only in an existing tmpfs mount with mode `0600`. Certificate and client-secret rendering plus negative conditions are covered by shell tests. This local work has no deployment or release authority; formal functional Gate B owner approval remains pending.
