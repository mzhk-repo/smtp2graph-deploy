...
2026-07-31 — Task 5.3 now separates build publication from deployment: build-plane PR/dev/tag paths validate and publish only, while a protected control-plane manual dispatch deploys an explicit immutable digest. Tag release evidence is Trivy, CycloneDX and checksum-backed without overwrite; production evidence attestation remains a protected-environment manual check. No registry push, GitHub Release, host mutation or live stack deployment was performed.
