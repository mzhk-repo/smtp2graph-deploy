...
- Temporary Task 3.2 source work exists only as a local build-plane diff and is not a valid delivery artifact. Before any further review, test or integration it must be converted into versioned control-plane patch assets and reapplied from this repository to an isolated worktree; direct build-plane edits must not be retained or promoted.
...
2026-07-29 — Control plane is now the mandatory source of truth for every gateway fork source patch. Build-plane changes must be versioned under `patches/smtp2graph/<upstream-version>/`, applied by reviewed automation to an isolated worktree and verified before commit; direct/untracked build checkout edits are disposable and cannot be used for review, release, deployment or Gate B. The existing local Task 3.2 diff must be converted into such assets before further use. Task 3.1 runtime verification remains complete with POSIX `/bin/sh` compatibility, explicit Docker Secret owner allowlisting and passing isolated Docker acceptance checks.
