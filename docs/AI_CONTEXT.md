...
- Task 5.1b dev/prod Swarm IaC is defined. The single canonical stack requires `smtp2graph_dev` or `smtp2graph_prod` according to its encrypted environment contract; host-side `SERVER_ENV=dev|prod` from `/etc/environment` must match. The idempotent legacy migration from `smtp2graph_nonproduction` and `/srv/smtp2graph/non-production` is explicit `--apply` only and refuses a running gateway or unsafe storage paths. Production host mutation additionally requires an approval context; no production host state was changed.
- Task 5.2 dev/prod Swarm orchestration is complete in the control plane. `scripts/deploy-orchestrator-swarm.sh` strictly parses only allowlisted stack inputs and Secret names without `source` or local `.env` fallback, accepts only immutable digests, and requires matching host `SERVER_ENV`. Production deploy requires tag, approval context and a matching exact control-plane SHA; fake-Docker regression covers dev idempotency, production guards and digest-only deploy. No live stack deployment was performed.
...
- Task 5.4 control plane is complete, while live development rehearsal remains blocked on two Task 5.3-approved digests, their release evidence, a declared queue-compatible pair and authorised host/mailbox access. `rehearse-deployment.sh` uses a temporary invalid Graph credential to create an SMTP-accepted queue item without delivery during upgrade, then rolls back and requires read-only mailbox proof after queue drain. The accepted recovery model is At-Least-Once: a duplicate is preferable to silent loss when Graph completion and queue cleanup are interrupted.
...
2026-08-03 — Task 5.4 adds a fail-closed development deployment rehearsal: exact-digest compatibility/evidence metadata, live read-only smoke checks, temporary invalid-credential queue creation, candidate upgrade, compatible rollback and non-secret evidence collection. ADR-0008 documents At-Least-Once recovery delivery. Local fake-Docker coverage passes; no live stack, Docker Secret, queue or Microsoft 365 action was performed.
