...
- Task 5.4 is limited to a single-release development smoke rehearsal. The exact fork digest `ghcr.io/mzhk-repo/smtp2graph-build@sha256:3d5069229059411988884913b6afba2d198c43e2440c1037e44d6aa54f172f65` has independently reviewed release artifacts in `deploy/config/queue-compatibility.yml`. Following storage, source-CIDR and encrypted-overlay remediation, a declarative development redeploy completed with exactly one desired `Running` task, SMTP `220`, reviewed Secret mount modes and a passing live network-policy check. The SOPS-backed deploy loader decrypts only into `/dev/shm`, then requires and overlays the complete private names-only Secret mapping, so rotation never relies on manually copied `*_SECRET_NAME` values. `GRAPH_SENDER_MAILBOX` is the single runtime sender and derives the global SMTP allowlist; `SMTP_USERS_TSV` retains its independent per-user sender scope. No-op redeploy evidence remains outstanding; upgrade, queue migration and rollback rehearsal additionally require a second independently reviewed queue-compatible fork release. ADR-0008 retains the At-Least-Once model for future rollback/recovery.
...
2026-08-10 — Task 5.4 development smoke passed after a declarative task revision incorporated the storage initializer, the development host converged on an encrypted overlay and the development source-CIDR contract included the local bridge probe. One desired task is `Running`, SMTP returns `220`, reviewed Secret mount modes and live nftables/network policy checks pass. `check-network-policy.sh` now takes an explicit overlay target and does not read deployment environment files. No production action, upgrade, queue migration or rollback rehearsal is implied.
