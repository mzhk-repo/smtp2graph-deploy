...
- Task 5.4 single-release development smoke rehearsal is complete. The exact fork digest `ghcr.io/mzhk-repo/smtp2graph-build@sha256:3d5069229059411988884913b6afba2d198c43e2440c1037e44d6aa54f172f65` has independently reviewed release artifacts in `deploy/config/queue-compatibility.yml`. Fresh deploy, no-op declarative redeploy with the same desired task ID, and repeated read-only smoke passed: exactly one desired task is `Running`, SMTP returns `220`, reviewed Secret mount modes and live encrypted-overlay/nftables policy checks pass. The SOPS-backed deploy loader decrypts only into `/dev/shm`, then requires and overlays the complete private names-only Secret mapping, so rotation never relies on manually copied `*_SECRET_NAME` values. `GRAPH_SENDER_MAILBOX` is the single runtime sender and derives the global SMTP allowlist; `SMTP_USERS_TSV` retains its independent per-user sender scope. Upgrade, queue migration and rollback rehearsal remain deferred until a second independently reviewed queue-compatible fork release. ADR-0008 retains the At-Least-Once model for future rollback/recovery.
- Task 6.1 gateway format delivery is complete with exactly one non-production sender mailbox and one allowlisted recipient: plain text, HTML/Unicode, To/CC, Reply-To, regular/inline attachments and a BCC-envelope case all passed with read-only recipient evidence on 2026-08-12. The BCC recipient was not exposed in the received message. The next profile is Matomo on the gateway host; its `/etc/hosts` and NSS resolution map the configured `SMTP_TLS_FQDN` (`smtp-int.pinokew.buzz`) to `10.138.131.45`. Moodle follows Matomo, pending its real VM STARTTLS, hostname-validation and AUTH-before-TLS-denial evidence.
...
2026-08-12 — Task 5.4 single-release development rehearsal is complete: fresh deploy and no-op redeploy retained one desired `Running` task, and repeated smoke confirmed SMTP `220`, reviewed Secret mounts and encrypted-overlay/nftables policy. The development-only nftables contract permits loopback access to the published SMTP listener for the read-only host smoke probe; the gateway's own source-IP and SMTP authentication policy remain enforced. No production action, upgrade, queue migration or rollback rehearsal is implied.
