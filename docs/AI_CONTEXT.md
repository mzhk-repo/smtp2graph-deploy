...
- Task 7.1 control-plane observability wiring is implemented but not yet accepted with live evidence. The gateway config enables port `9464` only on the encrypted overlay; the Swarm healthcheck uses `/readyz`, the local logging driver is bounded by size/file count, and `tests/observability/test-signals.sh` verifies endpoints, privacy-bounded metric names, process/auth/delivery/queue/storage signals and current JSON logs. VictoriaMetrics must still join the overlay and import `deploy/monitoring/smtp2graph-scrape.yml`; the central logging owner must separately verify 30-day time retention.
