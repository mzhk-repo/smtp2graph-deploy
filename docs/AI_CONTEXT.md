...
- Task 6.1 gateway format delivery is complete with exactly one non-production sender mailbox and one allowlisted recipient: plain text, HTML/Unicode, To/CC, Reply-To, regular/inline attachments and a BCC-envelope case all passed with read-only recipient evidence on 2026-08-12. The BCC recipient was not exposed in the received message. Moodle is now the next profile, pending its real STARTTLS, hostname-validation and AUTH-before-TLS-denial evidence.
