...
- Task 5.1 non-production control-plane IaC is complete. `deploy/swarm/stack.yml` defines one hardened stateful replica, external encrypted overlay, host-mode SMTP publish, versioned external Docker Secrets, Swarm Config-mounted runtime files, persistent `/data`, TCP healthcheck and Swarm-native restart/update policy. `bootstrap-swarm-host.sh` validates or explicitly applies only the reviewed non-production overlay, node label, storage and nftables prerequisites. No stack, firewall, storage or Docker state has been applied to a live host; a Task 5.3 image digest and authorised Swarm manager remain required.
