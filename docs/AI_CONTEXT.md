...
2026-08-03 — Task 5.3 now separates build publication from deployment: build-plane PR/dev/tag paths are thin shared-CI callers with no local Action/image uses, while a protected control-plane manual dispatch deploys an explicit immutable digest. Tag release evidence is Trivy, CycloneDX and checksum-backed without overwrite; production evidence attestation remains a protected-environment manual check. Legacy upstream workflows are quarantined as `.disabled` assets. No registry push, GitHub Release, host mutation or live stack deployment was performed.
