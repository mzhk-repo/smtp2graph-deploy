...
- Task 6.1 starts with gateway format delivery, using exactly one non-production sender mailbox and one allowlisted recipient. Plain text, HTML/Unicode, To/CC, Reply-To and attachment cases passed with read-only recipient evidence on 2026-08-12. Moodle remains the final profile and is blocked until the separate BCC-envelope case passes under the same one-recipient constraint.
