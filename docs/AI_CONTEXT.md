...
- Task 5.2 dev/prod Swarm orchestration is complete in the control plane. `scripts/deploy-orchestrator-swarm.sh` strictly parses only allowlisted stack inputs, env-derived immutable digest and Secret names without `source`; development may use local ignored `.env` fallback, while `.env.dev.enc` remains a SOPS reconciliation source. Before every deploy or rollback it invokes the shared idempotent `init-storage.sh`, which validates or creates the reviewed storage root and its direct queue/failed children. Production requires an explicit env-file, label, tag, approval context and matching exact control-plane SHA. Fake-Docker regression covers dev idempotency, digest-only deploy and production guards. No live stack deployment was performed.
...
- Task 5.4 наразі обмежено single-release development smoke rehearsal. Exact fork digest `ghcr.io/mzhk-repo/smtp2graph-build@sha256:3d5069229059411988884913b6afba2d198c43e2440c1037e44d6aa54f172f65` має independently reviewed release artifacts у `deploy/config/queue-compatibility.yml`; fresh deploy, no-op redeploy і read-only smoke можуть виконуватись лише на authorised development host. The SOPS-backed deploy loader decrypts only into `/dev/shm`, then requires and overlays the complete private names-only Secret mapping, so rotation never relies on manually copied `*_SECRET_NAME` values. `GRAPH_SENDER_MAILBOX` is the single runtime sender and derives the global SMTP allowlist; `SMTP_USERS_TSV` retains its independent per-user sender scope. Upgrade, queue migration і rollback rehearsal deferred до другого independently reviewed queue-compatible fork release. ADR-0008 зберігає At-Least-Once model для майбутнього rollback/recovery.
...
2026-08-08 — Task 5.4 deploy loader now overlays a complete private names-only Docker Secret mapping over SOPS-decrypted public deployment configuration and fails closed for missing or incomplete mappings. `GRAPH_SENDER_MAILBOX` is the canonical single-mailbox runtime sender; the global SMTP allowlist is derived from it, while `SMTP_USERS_TSV` preserves per-user scope. The development service smoke remains unaccepted; no production action is implied.
