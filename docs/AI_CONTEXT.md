...
- Task 5.3 workflow implementation is prepared across build and shared planes. PR validation has no package/deploy permissions; push to `dev` calls the shared workflow for development delivery; a version tag calls it for protected production delivery. The shared workflow generates exact-digest Trivy JSON, CycloneDX SBOM, OCI metadata and checksum-backed release evidence before tag-based deployment. The caller intentionally uses `mzhk-repo/shared-workflows/.github/workflows/shared-ci-cd.yml@main` by owner direction; its mutable reference remains an explicit security exception.
...
2026-08-01 — Task 5.1b normalizes the host contract to `SERVER_ENV=dev|prod`, encrypted `development|production` contracts, `smtp2graph_dev`/`smtp2graph_prod` labels and `/srv/smtp2graph/dev|prod` paths. Task 5.3 prepares a dev/tag split caller and shared exact-digest Trivy, CycloneDX, OCI and checksum-backed release evidence; no registry push, GitHub Release, host mutation or live stack deployment was performed.
