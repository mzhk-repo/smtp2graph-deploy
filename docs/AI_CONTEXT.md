...
- Task 2.1 ADR baseline is complete. `docs/adr/ADR-0001` through `ADR-0007` record the SMTP-to-Graph boundary, initial gateway candidate, Swarm topology, sender mailbox, Graph mailbox scope, secret boundary, and cold-recovery model. Task 2.5 rejected upstream SMTP2Graph v1.1.5 in ADR-0002; all functional evidence for fork revision `0fbb699` is complete. Phase 3 remains blocked only pending formal functional Gate B approval.
...
- Task 4.2 non-production IaC baseline is present but is not applied. It defines host-mode SMTP ingress, encrypted-overlay validation, nftables allowlist policy, deterministic TLS Docker Secret reconciliation and a Cloudflare DNS-01 Certbot issuer isolated in `/opt/Traefik`. The issuer does not use Traefik `acme.json`; DNS/firewall/Docker state remain unchanged until Gate B approval and explicit non-production inputs are supplied.
...
2026-07-29 — Task 3.2 policy/storage changes are represented by control-plane assets `005-smtp-policy-and-storage-guards.patch`, `006-rejected-submission-queue-isolation.patch` and `007-receive-log-privacy.patch`. All seven v1.1.5 assets replayed from clean upstream in an isolated worktree; local build, 6 unit tests and 35 receive tests passed. The M365 suite is explicit opt-in through `--env-file`, so local patch qualification does not implicitly read build-plane `.env` or perform external calls. Successful automation removes its temporary worktree and `upgrade/vX.Y.Z` branch; failed runs preserve them for review. The current local build-plane diff is still disposable and must not be promoted directly. The control-plane runtime template enforces SMTP AUTH plus non-empty source-IP/sender allowlists and strict Docker Secret TSV user records with lowercased sender checks and redacted diagnostics. It activates the fork capacity guard with validated persistent storage root, max-byte and threshold settings, and the session/rate guards with 5 sessions/IP plus 30 messages/min/client defaults for every client, including Moodle. Asset `005` proves `451` before `DATA` without a new queue payload at threshold and when a client exceeds its rate limit. Asset `007` proves that runtime-generated body and attachment markers are absent from gateway console and local Winston file logs. The replayed receive integration tests prove that oversize, unauthenticated and sender-denied submissions create no queue payload. Task 3.3 `make test-local` extends that evidence through an ephemeral local patched image and isolated Compose mock Graph, covering positive SMTP delivery and restart recovery without M365 access. Failed payload retention defaults to dry-run, purges only files at least seven days old from the validated fixed `failed` child and never touches queue data.
