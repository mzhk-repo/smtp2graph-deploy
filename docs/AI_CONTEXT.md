...
- Task 6.1 starts with gateway format delivery, using exactly one non-production sender mailbox and one allowlisted recipient. Moodle is the final profile: its STARTTLS/authentication compatibility check is blocked until plain text, HTML/Unicode, recipient-header, Reply-To and attachment cases have passing SMTP and read-only delivery evidence.
