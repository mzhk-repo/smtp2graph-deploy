...
- **Implementation Steps:** build-plane PR/dev/tag paths are thin shared-CI callers that validate, build and push only; caller-specific npm checks are a reviewed script, while all Action/image `uses:` are centralized in shared CI/CD. Tag path generates exact-digest Trivy/SBOM/checksum Release evidence without overwrite. Control-plane protected manual dispatch accepts only `IMAGE@sha256`, production tag and manual evidence attestation, then invokes the reviewed Swarm adapter. Pin Actions/images except the owner-approved `shared-ci-cd.yml@main` exception; retain hashes and immutable artifact links.
