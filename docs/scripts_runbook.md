...
- Inputs: explicit build repo, upstream tag or `--latest`, reviewed patch bundle, optional ignored M365 env file, optional safe local `--test-image NAME:TAG`, optional `--target-branch BRANCH`, optional `--push`. Без explicit `--env-file` script не читає build-plane `.env` і не запускає M365 suite.
