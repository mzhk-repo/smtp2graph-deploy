...
if grep -Fq 'smtp2graph_prod' "$nft_file"; then
  grep -Fq 'tcp dport 2525 ip saddr @smtp2graph_prod_smtp_clients accept' "$nft_file" || die 'nftables allowlist rule is missing.'
else
...
nft list ruleset | grep -Eq 'smtp2graph(_prod)?_smtp_clients' || die 'smtp2graph nftables policy is not loaded. Run bootstrap-swarm-host.sh --apply to load the policy.'
