...
grep -Fq 'mode: host' "$stack"
if grep -Fq 'mode: ingress' "$stack"; then
  printf 'ERROR: routing mesh publish is forbidden.\n' >&2
  exit 1
fi
grep -Fq 'target: 2525' "$stack"
grep -Fq 'mode: 0400' "$stack"
grep -Fq 'type filter hook input priority filter - 1; policy accept;' "$nft"
grep -Fq 'iifname "lo" tcp dport 2525 accept' "$nft"
grep -Fq 'tcp dport 2525 ip saddr @smtp2graph_smtp_clients accept' "$nft"
grep -Fq 'tcp dport 2525 drop' "$nft"
if grep -Eq '0\.0\.0\.0/0|::/0' "$nft"; then
...
grep -Fq '10.42.0.0/24,192.168.20.4/32' "$rendered"
printf '%s\n' 'SMTP_ALLOWED_SOURCE_CIDRS=100.64.0.4/32' >"$env_file"
"$renderer" --env-file "$env_file" "$rendered"
grep -Fq '100.64.0.4/32' "$rendered"
...
"$renderer" --env-file "$env_file" "$rendered"
grep -Fq 'table inet smtp2graph_prod {' "$rendered"
grep -Fq 'smtp2graph_prod_smtp_clients' "$rendered"
